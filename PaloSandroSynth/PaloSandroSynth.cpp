...
: Plugin(info, MakeConfig(kNumParams, kNumPresets))
{
  ...
  mLayoutFunc = [&](IGraphics* pGraphics) {
    ...
    pGraphics->AttachPanelBackground(IColor(255, 96, 150, 186)); /* TASK_01 */
...
#if IPLUG_DSP
void PaloSandroSynth::ProcessBlock(sample** inputs, sample** outputs, int nFrames)
{
...
void PaloSandroSynth::ProcessMidiMsg(const IMidiMsg& msg)
{
  mSynth.AddMidiMsgToQueue(msg);
}

void PaloSandroSynth::OnReset()
{
  mSynth.SetSampleRateAndBlockSize(GetSampleRate(), GetBlockSize());
}

void PaloSandroSynth::OnParamChange(int paramIdx)
{
