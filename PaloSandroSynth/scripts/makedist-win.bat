...
if %DEMO_ARG% == "demo" (
  echo Making PaloSandroSynth Windows DEMO VERSION distribution ...
  set DEMO=1
) else (
  echo Making PaloSandroSynth Windows FULL VERSION distribution ...
  set DEMO=0
)
...
REM - msbuild PaloSandroSynth-app.vcxproj /p:configuration=release /p:platform=win32

REM echo Building 32 bit binaries...
REM msbuild PaloSandroSynth.sln /p:configuration=release /p:platform=win32 /nologo /verbosity:minimal /fileLogger /m /flp:logfile=build-win.log;errorsonly 

REM echo Building 64 bit binaries...
REM add projects with /t to build VST2 and AAX
msbuild PaloSandroSynth.sln /t:PaloSandroSynth-app;PaloSandroSynth-vst3 /p:configuration=release /p:platform=x64 /nologo /verbosity:minimal /fileLogger /m /flp:logfile=build-win.log;errorsonly;append
...
REM --wraptool sign --verbose --account XXXXX --wcguid XXXXX --keyfile XXXXX.p12 --keypassword XXXXX --in .\build-win\aax\bin\PaloSandroSynth.aaxplugin\Contents\Win32\PaloSandroSynth.aaxplugin --out .\build-win\aax\bin\PaloSandroSynth.aaxplugin\Contents\Win32\PaloSandroSynth.aaxplugin
REM --wraptool sign --verbose --account XXXXX --wcguid XXXXX --keyfile XXXXX.p12 --keypassword XXXXX --in .\build-win\aax\bin\PaloSandroSynth.aaxplugin\Contents\x64\PaloSandroSynth.aaxplugin --out .\build-win\aax\bin\PaloSandroSynth.aaxplugin\Contents\x64\PaloSandroSynth.aaxplugin
...
echo Making Installer ...
  ...
  REM :32-Bit-is
  REM REM "%ProgramFiles%\Inno Setup 6\iscc" /Q ".\installer\PaloSandroSynth.iss"
  REM goto END-is

  REM :64-Bit-is
  "%ProgramFiles(x86)%\Inno Setup 6\iscc" /Q ".\installer\PaloSandroSynth.iss"
  REM goto END-is

  REM :END-is

  REM - Codesign Installer for Windows 8+
  REM -"C:\Program Files (x86)\Microsoft SDKs\Windows\v7.1A\Bin\signtool.exe" sign /f "XXXXX.p12" /p XXXXX /d "PaloSandroSynth Installer" ".\installer\PaloSandroSynth Installer.exe"

  REM -if %1 == 1 (
  REM -copy ".\installer\PaloSandroSynth Installer.exe" ".\installer\PaloSandroSynth Demo Installer.exe"
  REM -del ".\installer\PaloSandroSynth Installer.exe"
  REM -)
