/* Declares the PaloSandroSynth Audio Worklet Node */

class PaloSandroSynthController extends WAMController
{
  constructor (actx, options) {
    ...
    super(actx, "PaloSandroSynth", options);
  }

  static importScripts (actx) {
    var origin = "https://francescocretti.github.io/palo-sandro-synth/";

    return new Promise( (resolve) => {
      actx.audioWorklet.addModule(origin + "scripts/PaloSandroSynth-wam.js").then(() => {
      actx.audioWorklet.addModule(origin + "scripts/wam-processor.js").then(() => {
      actx.audioWorklet.addModule(origin + "scripts/PaloSandroSynth-awp.js").then(() => {
        resolve();
      }) }) });
    })
  }
